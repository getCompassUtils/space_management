...
abstract class BasePlan extends \Tariff\Plan\BasePlan implements MemberCount {

	/** @var int уникальный идентификатор плана */
	public const PLAN_ID = 1000;

	/** @var int уникальный идентификатор плана */
	public const PLAN_TYPE = 1;

	protected const _LIMIT_OPTION_KEY           = OptionLimit::OPTION_NAME;
	protected const _EXTEND_POLICY_OPTION_KEY   = OptionExtendPolicy::OPTION_NAME;
	protected const _RESTRICT_POLICY_OPTION_KEY = OptionRestrictPolicy::OPTION_NAME;

	public const ERROR_PLAN_WAS_CHANGED                = 100_00_0_10; // план не соответствует ожидаемым параметрам
	public const ERROR_PLAN_UNEXPECTED_EXPIRATION_DATE = 100_00_0_11; // срок действия плана не соответствует ожидаемому
	...
	protected const _DEFAULT_OPTION_DATA = [
		...
		self::_EXTEND_POLICY_OPTION_KEY   => [
			"rule" => OptionExtendPolicy::FREE
		],
	...
	protected OptionExtendPolicy   $_option_extend_policy;   // политика расширения
	...
	protected function __construct(
		...
		OptionExtendPolicy   $option_extend_policy,
		...
		$this->_option_extend_policy   = $option_extend_policy;
	...
	public function _hasOptionChanges():bool {
		...
		$is_same = $this->_option_extend_policy->isSame($this->_original_dynamic->option_extend_policy)
			&& $this->_option_limit->isSame($this->_original_dynamic->option_limit)
	 ...
	 * @inheritDoc
	 */
	#[\JetBrains\PhpStorm\ArrayShape([self::_LIMIT_OPTION_KEY => "int[]", self::_EXTEND_POLICY_OPTION_KEY => "string[]", self::_RESTRICT_POLICY_OPTION_KEY => "int[]"])]
	public function _getOptionSaveData():array {

		return [
			static::_LIMIT_OPTION_KEY           => $this->_option_limit->export(),
			static::_EXTEND_POLICY_OPTION_KEY   => $this->_option_extend_policy->export(),
	...
	public function isFree(int $time):bool {
		...
		return $this->isTrial($time) || $this->_active_till === 0 || $this->_option_limit->isFree();
	}
	...
	protected function _arrangeAlteration(Alteration $alteration, Circumstance $circumstance, Action $action, Dynamic $expected):Alteration {
		...
		// проверяем, что текущая политика расширения совпадает с ожидаемой
		if (isset($alteration->expected_option_extend_policy) && !$this->_option_extend_policy->isSame($alteration->expected_option_extend_policy)) {
			$alteration->availability->arrange(new AlterationAvailability(AlterationAvailability::UNAVAILABLE_UNAPPROPRIATED, 1, "extend policy not equal to expected"));
		}
		...
		// если альтерация разрешена при активном
		// пробном периоде, то разрешаем бесплатную активацию
		if ($alteration->isFreeOnActiveTrial() && $this->isTrial($action_time)) {
			$alteration->availability->arrange(new AlterationAvailability(AlterationAvailability::AVAILABLE_WHILE_TRIAL));
		}

		// если альтерация разрешена при возможности активировать
		// пробный период, то разрешаем бесплатную активацию;
		// обычно, в результате должен произойти переход в пробный период
		if ($alteration->isFreeOnActiveAvailable() && $this->isTrialAvailable($action_time)) {
			$alteration->availability->arrange(new AlterationAvailability(AlterationAvailability::AVAILABLE_WHILE_TRIAL));
		}

		// если для альтерации требуется доступный пробный период
		if ($alteration->isActiveTrialRequired() && !$this->isTrialAvailable($action_time)) {
			$alteration->availability->arrange(new AlterationAvailability(AlterationAvailability::AVAILABLE_WHILE_TRIAL));
		}
		...
		$alteration->availability->arrange(
			...
			$this->_option_extend_policy->resolveAlterationAvailability($circumstance, $action, $this->_last_dynamic, $expected),
	...
	protected function _applyAlteration(Dynamic $expected_dynamic):\Tariff\Plan\AlterationResult {
		...
		$this->_option_extend_policy   = $expected_dynamic->option_extend_policy;
	...
	protected function _fillAlteration(Alteration $alteration, int $relative_time):Alteration {
		...
		if (isset($alteration->option_limit) && $alteration->option_limit->isFree() && !$this->isTrial($relative_time)) {
	...
	protected function _fillAction(int $method, Alteration $alteration, int $relative_time):Action {
		...
		if ($this->isTrial($relative_time)) {
			$trial_state = Action::TRIAL_STATE_ACTIVE;
		} elseif ($this->isTrialAvailable($relative_time)) {
			$trial_state = Action::TRIAL_STATE_AVAILABLE;
		} else {
			$trial_state = Action::TRIAL_STATE_NONE;
		}
		...
		return new Action($method, $alteration, $trial_state, $active_state_list ?? [Action::ACTIVE_STATE_NONE], $relative_time);
	}
	...
	protected function _fillDynamic(Alteration $alteration, Circumstance $circumstance, Action $action):Dynamic {
		...
		$opt_extend_policy   = $alteration->option_extend_policy ?? null;
		...
		$dynamic = new Dynamic($expected_active_till, $expected_free_active_till, $opt_extend_policy, $opt_limit, $opt_restrict_policy);
		...
		$opt_extend_policy   = $opt_extend_policy ?? $this->_option_extend_policy->makeAlterationReplacer($circumstance, $action, $this->_last_dynamic, $dynamic);
		...
		return new Dynamic($expected_active_till, $expected_free_active_till, $opt_extend_policy, $opt_limit, $opt_restrict_policy);
	}
	...
	public function isTrial(int $relative_time):bool {
		...
		return $this->_active_till !== 0 && $this->_active_till > $relative_time && $this->_option_extend_policy->isTrial($relative_time);
	}

	/**
	 * @inheritDoc
	 */
	public function isTrialAvailable(int $relative_time = 0):bool {

		return $this->_option_extend_policy->isTrialAvailable();
	}
	...
	public function getExtendPolicyRule():string {
		...
		return $this->_option_extend_policy->getRule();
	}
	...
	public static function fromData(int $active_till, int $free_active_till, array $option_data_list):static {
		...
		$option_extend_policy   = new OptionExtendPolicy(...static::_resolveOptionData(static::_EXTEND_POLICY_OPTION_KEY, $option_data_list));
		...
		return new static($active_till, $free_active_till, $option_limit, $option_extend_policy, $option_restrict_policy);
	}
	...
	public static function fromRows(array $row_list):static {
		...
		$option_extend_policy_argument_list   = static::_resolveOptionData(static::_EXTEND_POLICY_OPTION_KEY, []);
		...
		$option_extend_policy_load_fn   = OptionExtendPolicy::makeLoadFn($option_extend_policy_argument_list);
		...
		foreach ($row_list as $row) {
			...
			$option_extend_policy_argument_list   = $option_extend_policy_load_fn($row);
		...
		$plan_argument_list["option_data_list"][static::_EXTEND_POLICY_OPTION_KEY]   = $option_extend_policy_argument_list;
